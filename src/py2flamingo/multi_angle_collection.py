...
def multi_angle_collection(
    ...
    for i in range(int(360/angle_step_size_deg)):
        ...
        xyzr_bottom = calc.bounding_point_from_angle(bottom_points, angle_step_size_deg*i)
        center_point = calc.find_center(xyzr_top, xyzr_bottom, shift=frame_size*image_pixel_size_mm/2)
        ...
        # ).replace('\\', '/')
        wf_dict["Experiment Settings"]["Save image directory"] = f"{sample_name}_{int(current_angle)}"
