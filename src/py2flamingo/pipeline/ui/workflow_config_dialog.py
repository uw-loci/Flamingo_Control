...
from py2flamingo.utils.workflow_parser import dict_to_workflow_text, parse_workflow_file
...
class PipelineWorkflowConfigDialog(PersistentDialog):
    ...
    def __init__(self, app=None, template_file: str = "", parent=None):
        """
        Args:
    ...
    def _load_from_template(self, path: str):
        ...
        exposure_str = cam.get(
            "Exposure time (us)", exp.get("Exposure time (us)", "10000")
        )
        try:
            cam_settings["exposure_us"] = float(str(exposure_str).replace(",", ""))
        except (ValueError, TypeError):
            cam_settings["exposure_us"] = 10000.0
        frame_rate_str = cam.get(
            "Frame rate (f/s)", exp.get("Frame rate (f/s)", "40.0")
        )
        try:
        ...
        try:
            ...
            z_range_mm = 0.25
        z_step_um = (
            (z_range_mm / max(num_planes - 1, 1)) * 1000.0 if num_planes > 1 else 5.0
        )
        try:
        ...
        if "Save max projection" in exp:
            save_settings["save_mip"] = (
                str(exp["Save max projection"]).lower() == "true"
            )
        if "Display max projection" in exp:
            save_settings["display_mip"] = (
                str(exp["Display max projection"]).lower() == "true"
            )
        if "Save to subfolders" in exp:
            save_settings["save_subfolders"] = (
                str(exp["Save to subfolders"]).lower() == "true"
            )
        if "Work flow live view enabled" in exp:
            save_settings["live_view"] = (
                str(exp["Work flow live view enabled"]).lower() == "true"
            )
        if "Comments" in exp:
    ...
    def _build_workflow_dict(self) -> Dict[str, Any]:
        ...
        experiment = {
            ...
            "Work flow live view enabled": save_dict.get(
    ...
    def _on_save(self):
        ...
        if not name:
            QMessageBox.warning(self, "Name Required", "Please enter a workflow name.")
            return
