...
class SampleViewDataRunner(AbstractNodeRunner):
    ...
    def run(
        ...
        if not voxel_storage:
            raise RuntimeError(
                "Voxel storage service not available — is the 3D viewer open?"
            )

        volumes: Dict[int, np.ndarray] = {}
        for ch_id in selected_channels:
            try:
                if hasattr(voxel_storage, "has_data") and not voxel_storage.has_data(
                    ch_id
                ):
                    continue
        ...
        logger.info(
            f"Sample View Data: read {len(volumes)} channels: {list(volumes.keys())}"
        )
