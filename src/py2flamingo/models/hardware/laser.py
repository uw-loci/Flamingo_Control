...
class LaserCalibration:
    ...
    power_calibration: Dict[float, float] = field(
...
class PulseSettings:
    ...
    def __post_init__(self):
        ...
        validate_range(
            self.frequency_hz, min_val=0.1, max_val=1e9, field_name="frequency_hz"
        )
        validate_range(
            self.duty_cycle_percent,
            min_val=0.1,
            max_val=100.0,
            field_name="duty_cycle_percent",
        )
...
class LaserSettings(ValidatedModel):
    ...
    def validate(self) -> None:
        ...
        if self.power_setpoint_mw < 0:
            raise ValidationError(
                f"Power cannot be negative: {self.power_setpoint_mw} mW"
            )
        ...
        if self.mode == LaserMode.MODULATED:
            if self.modulation_frequency_hz is None:
                raise ValidationError(
                    "Modulation frequency required for modulated mode"
                )
...
class LaserSafetyStatus:
    ...
    def is_safe_to_operate(self) -> bool:
        ...
        """
        return (
            self.interlock_closed
            and self.key_switch_on
            and self.temperature_ok
            and self.coolant_flow_ok
        )
...
class Laser(ValidatedModel):
    ...
    def validate(self) -> None:
        ...
        if not self.power_limits.is_valid_power(
            self.settings.power_setpoint_mw, use_safe_limit=False
        ):
            raise ValidationError(
    ...
    @classmethod
    def create_default(
        cls, name: str, wavelength_nm: float, channel_id: int = 0
    ) -> "Laser":
