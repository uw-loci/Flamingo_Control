...
class NapariViewer(ViewerInterface):
    ...
    def __init__(self, viewer: Optional["napari.Viewer"] = None):
        if napari is None:
            raise ImportError(
                "Napari is not installed. Install napari or use a different viewer adapter."
            )
        self.viewer = viewer or napari.Viewer()
        self._layer_name = "Flamingo Live"

    def display_image(
        self, image: Any, title: str = "", metadata: Dict[str, Any] = None
    ):
        """
        ...
        try:
            ...
            if self._layer_name in self.viewer.layers:
                ...
                self.viewer.add_image(
                    image, name=title or self._layer_name, metadata=metadata or {}
                )
        except Exception:
