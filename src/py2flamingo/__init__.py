...
class FlamingoController(QMainWindow):
    ...
    def check_start_position(self):
        ...
        if not os.path.exists(file_path):  # Check if the file does not exist
            ...
            positions_dict = text_to_dict(os.path.join('microscope_settings',f'{self.instrument_name}_start_position.txt'))
            self.start_position = [float(positions_dict[self.instrument_name]['x(mm)']), float(positions_dict[self.instrument_name]['y(mm)']),float(positions_dict[self.instrument_name]['z(mm)']),float(positions_dict[self.instrument_name]['r(°)'])] 
    ...
    def check_metadata_file_and_connect(self):
        ...
        metadata_dict = text_to_dict(file_path)  # Convert the metadata text file to a dictionary
        self.IP = metadata_dict['Instrument']['Type']['Microscope address'].split(' ')[0]  # Get the IP address from the dictionary
        self.port = int(metadata_dict['Instrument']['Type']['Microscope address'].split(' ')[1])  # Get the port from the dictionary
        ...
        live_listen_thread_var, command_listen_thread_var, send_thread_var, processing_thread_var = mc.create_threads(
    ...
    def create_GUI_layout(self):
        ...
        go_to_position_button.setToolTip('Moves to the x,y,z,r coordinates listed above.')
        go_to_position_button.clicked.connect(self.go_to_position)
        set_home_button = QPushButton("Set 'Home' on Mac", self)
        set_home_button.setToolTip("Updates the values for the 'Home' position on the microscope\nusing the values displayed under the image.")
        ...
        kill_button.setToolTip('Completely close the program and end connections to the microscope.')
        kill_button.clicked.connect(self.close_program)
        cancel_button = QPushButton('Cancel current process', self)
        cancel_button.setToolTip('Cancels any process that checks for the terminate_event being set via terminate_event.set()')
    ...
    def go_to_position(self):
        ...
        xyzr_init = [self.field_x_mm.text(),self.field_y_mm.text(),self.field_z_mm.text(),self.field_r_deg.text()]
    ...
    def locate_sample_action(self):
        ...
        xyzr_init = [self.field_x_mm.text(),self.field_y_mm.text(),self.field_z_mm.text(),self.field_r_deg.text()]
        dict_to_text(os.path.join('microscope_settings', f'{self.instrument_name}_start_position.txt'), updated_position)
        # Create a thread for the locate_sample function
        locate_sample_thread = Thread(target=locate_sample, args=(self.connection_data, xyzr_init, visualize_event,
    ...
    def close_program(self):
        ...
        close_thread = Thread(target = mc.close_connection, args=(nuc_client, live_client,live_listen_thread_var, 
    ...
    def take_snapshot(self):
        ...
        take_snapshot_thread = Thread(target=take_snapshot, args=(self.connection_data, xyzr_init, visualize_event,
                  image_queue,command_queue,stage_location_queue, send_event, laser_value, power_percent_value))
        # Set the thread as a daemon (exits when the main program exits)
    ...
    def set_home_action(self):
        ...
        xyzr_init = [float(self.current_x.text().split(' ')[1]),float(self.current_y.text().split(' ')[1]),float(self.current_z.text().split(' ')[1]),float(self.current_r.text().split(' ')[1])]
        print(f"New 'Home' coordinates {xyzr_init}")

        #all functions must be run on a separate thread, targetting a function, including its arguments, and then start()
        set_home_thread = Thread(target=set_home, args=(self.connection_data, xyzr_init, command_queue, other_data_queue,send_event))
        # Set the thread as a daemon (exits when the main program exits)
