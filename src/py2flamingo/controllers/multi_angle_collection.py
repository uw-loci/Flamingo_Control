...
class MultiAngleController:
    ...
    def run_collection(
        ...
        try:
            ...
            base_workflow = self._configure_base_workflow(
                base_workflow, sample_name, comment, overlap_percent
            )

            # Run collection at each angle
            for angle in self.current_collection.angles:
                self._collect_at_angle(angle, bounds, base_workflow, sample_name)
    ...
    def _load_sample_bounds(self, sample_name: str) -> List[dict]:
        ...
        if top_file.exists():
            ...
            if bottom_file.exists():
                ...
                bounds_list.append({"top": top_points, "bottom": bottom_points})

        return bounds_list

    def _configure_base_workflow(
        self, workflow: dict, sample_name: str, comment: str, overlap_percent: float
    ) -> dict:
    ...
    def _collect_at_angle(
        self, angle: float, bounds: List[dict], base_workflow: dict, sample_name: str
    ):
        ...
        if bounds:
            ...
            top_at_angle = calc.bounding_point_from_angle(bound_data["top"], angle)
            bottom_at_angle = calc.bounding_point_from_angle(
                bound_data["bottom"], angle
            )
        else:
        ...
        if self.connection_service and hasattr(
            self.connection_service, "get_camera_info"
        ):
        ...
        angle_workflow["Experiment Settings"][
            "Comments"
