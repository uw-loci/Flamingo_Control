...
def find_sample_at_angle(
    connection_data,
    sample_name: str,
    wf_dict: tuple,
    z_search_depth_mm: float,
    z_init_mm: float,
    image_pixel_size_mm: float,
    frame_size: int,
    xyzr,
    wf_zstack: str,
    command_queue,
    ...
    wf_dict = scope.calculate_zplanes(
        wf_dict, z_step_depth_mm, FRAMERATE, plane_spacing
    )
...
def trace_ellipse(
    ...
    wf_dict = scope.laser_or_LED(
        wf_dict, laser_channel, laser_setting, LED_off, LED_on, True
    )
    ...
    wf_dict = txt.calculate_zplanes(
        wf_dict, z_search_depth_mm, FRAMERATE, plane_spacing
    )
    ...
    z_init_mm = np.mean(
        [
            float(scope_settings["Stage limits"]["Soft limit max z-axis"]),
            float(scope_settings["Stage limits"]["Soft limit min z-axis"]),
        ]
    )
    ########################################
    ...
    for i in range(1, int(360 / angle_step_size_deg)):
        ...
        top_bounds_mm, bottom_bounds_mm = find_sample_at_angle(
            connection_data,
            sample_name,
            wf_dict,
            z_search_depth_mm,
