...
def wire_image_controls_signals(image_controls_window, camera_live_viewer):
    ...
    image_controls_window.rotation_changed.connect(camera_live_viewer.set_rotation)
    ...
    image_controls_window.colormap_changed.connect(camera_live_viewer.set_colormap)
    image_controls_window.intensity_range_changed.connect(
        camera_live_viewer.set_intensity_range
    )
    image_controls_window.auto_scale_changed.connect(camera_live_viewer.set_auto_scale)
    image_controls_window.zoom_changed.connect(camera_live_viewer.set_zoom)
    image_controls_window.crosshair_changed.connect(camera_live_viewer.set_crosshair)
    logger.debug("Connected image controls to camera live viewer")


def wire_connection_signals(
    connection_view,
    on_established,
    on_closed,
    on_settings_loaded,
    ...
    if hasattr(connection_view, "connection_established"):
        connection_view.connection_established.connect(lambda: on_established())
        logger.debug("Connected connection_established to enhanced stage control")
    if hasattr(connection_view, "connection_closed"):
        connection_view.connection_closed.connect(lambda: on_closed())
    ...
    if hasattr(connection_view, "settings_loaded"):
        connection_view.settings_loaded.connect(lambda: on_settings_loaded())
        logger.debug("Connected settings_loaded to position query handler")


def wire_workflow_signals(
    workflow_view,
    status_indicator_service,
    position_controller,
    connection_view,
    ...
    if hasattr(workflow_view, "set_position_callback") and position_controller:
        workflow_view.set_position_callback(position_controller.get_current_position)
    ...
    if hasattr(connection_view, "connection_closed") and hasattr(
        workflow_view, "update_for_connection_state"
    ):
        connection_view.connection_closed.connect(
...
def wire_acquisition_lock_signals(
    acquisition_started, acquisition_stopped, stage_control_view
):
