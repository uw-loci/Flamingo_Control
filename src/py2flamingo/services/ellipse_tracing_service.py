...
class EllipseTracingService:
    ...
    def predict_point_on_ellipse(
        self, ellipse: EllipseParameters, angle_deg: float
    ) -> Tuple[float, float]:
        """
    ...
    def interpolate_ellipse_trajectory(
        self,
    ...
    def evaluate_ellipse_fit_quality(
        self, ellipse: EllipseParameters, points: List[Tuple[float, float]]
    ) -> float:
        """
    ...
    def merge_ellipse_data(
        self, ellipses: List[EllipseParameters], weights: Optional[List[float]] = None
    ) -> Optional[EllipseParameters]:
        """
        ...
        sin_sum = sum(
            np.sin(np.radians(e.rotation)) * w for e, w in zip(ellipses, weights)
        )
        cos_sum = sum(
            np.cos(np.radians(e.rotation)) * w for e, w in zip(ellipses, weights)
        )
    ...
    def refine_ellipse_fit(
        ...
        for i in range(iterations):
            ...
            # Remove outliers
            current_points = [
                p for i, p in enumerate(current_points) if i not in outliers
            ]
