...
class ConnectionManager:
    ...
    def connect(self):
        ...
        try:
            ...
            self.connected = True
            self.logger.info(
                f"Connected to microscope at {self.ip_address}:{self.port}"
            )
    ...
    def _start_threads(self):
        ...
        if not self.thread_manager.start_thread(
            name="command-sender", target=self._command_sender_loop, daemon=True
        ):
        ...
        if not self.thread_manager.start_thread(
            name="response-receiver", target=self._response_receiver_loop, daemon=True
        ):
