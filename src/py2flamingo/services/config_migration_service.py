...
class ConfigMigrationService:
    ...
    def migrate_all(self, dry_run: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if not dry_run:
                self._write_yaml(
                    self.configs_dir / "microscope_hardware.yaml", hardware_config
                )
                self._write_yaml(
                    self.configs_dir / "application_settings.yaml", app_config
                )
        ...
        if "Hard limit max x-axis" in data:
            hardware["stage"]["limits"] = {
                ...
                "r_axis": {
                    ...
                    "soft_min_degrees": float(
                        data.get("Soft limit min theta-axis", -360)
                    ),
                    "soft_max_degrees": float(
                        data.get("Soft limit max theta-axis", 360)
                    ),
                    "resolution_degrees": 0.001,
                    "speed_degrees_per_sec": float(
                        data.get("Default velocity theta-axis", 50)
                    ),
                },
            }
                ...
                filters.append(
                    {
                        ...
                        "description": data.get(
                            f"Filter wheel description {i}", f"Position {i}"
                        ),
    ...
    def _extract_microscope_json_settings(self, hardware: Dict):
        ...
        if json_path.exists():
            ...
            if "stage_limits" in json_data and "limits" not in hardware.get(
                "stage", {}
            ):
    ...
    def _extract_workflow_directories(self, app_settings: Dict):
        ...
        if snapshot_path.exists():
            ...
            if "File location" in workflow_data.get("Experiment Settings", {}):
                ...
                # Map to microscope
                if "MSN_LS" in path:
                    app_settings["workflow"]["directories"]["microscope_output_dirs"][
                        "n7"
                    ] = path
                elif "ctlsm1" in path:
                    app_settings["workflow"]["directories"]["microscope_output_dirs"][
    ...
    def _get_default_application_settings(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "user_interface": {"appearance": {"theme": "dark", "font_size": 10}},
            "workflow": {
                "directories": {
            ...
            "positions": {"history": {"max_positions": 100, "display_count": 20}},
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Migrate Flamingo Control configurations"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--path", type=str, help="Base path of Flamingo Control installation"
    )
