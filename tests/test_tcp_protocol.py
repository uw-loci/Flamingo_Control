...
from py2flamingo.core.tcp_protocol import CommandCode, ProtocolDecoder, ProtocolEncoder
...
class TestProtocolEncoder(unittest.TestCase):
    ...
    def test_encode_command_with_status(self):
        ...
        cmd = self.encoder.encode_command(CommandCode.CMD_WORKFLOW_STOP, status=42)
    ...
    def test_encode_command_with_parameters(self):
        ...
        cmd = self.encoder.encode_command(CommandCode.CMD_WORKFLOW_START, params=params)
    ...
    def test_encode_command_with_too_many_params_raises_error(self):
        ...
        with self.assertRaises(ValueError) as ctx:
            self.encoder.encode_command(CommandCode.CMD_WORKFLOW_START, params=params)
    ...
    def test_encode_command_with_value(self):
        ...
        cmd = self.encoder.encode_command(
            CommandCode.CMD_STAGE_POSITION_SET, value=value
        )
    ...
    def test_encode_command_with_data_string(self):
        ...
        data = b"test data"
        cmd = self.encoder.encode_command(CommandCode.CMD_WORKFLOW_START, data=data)
    ...
    def test_encode_command_with_str_data(self):
        ...
        data = "test string"
        cmd = self.encoder.encode_command(
            CommandCode.CMD_WORKFLOW_START, data=data.encode("utf-8")
        )
    ...
    def test_encode_command_with_invalid_data_type_raises_error(self):
        ...
        with self.assertRaises(ValueError):
            self.encoder.encode_command(
                CommandCode.CMD_WORKFLOW_START, data=12345  # Not bytes or str
            )
...
class TestProtocolDecoder(unittest.TestCase):
    ...
    def test_decode_command_extracts_status(self):
        ...
        cmd = self.encoder.encode_command(CommandCode.CMD_WORKFLOW_START, status=99)
        result = self.decoder.decode_command(cmd)
    ...
    def test_decode_encode_roundtrip(self):
        ...
        cmd = self.encoder.encode_command(
            code=code, status=status, params=params, value=value, data=data
        )
