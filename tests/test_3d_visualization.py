...
def simulate_frame_data(z_index=0, num_z_planes=20):
    ...
    frame[:, :, 1] = np.clip(
        15000 * z_modulation * np.exp(-dist / 80), 0, 65535
    ).astype(np.uint16)
...
def test_window():
    ...
    for name, x_mm, y_mm, z_mm in test_positions:
        ...
        print(
            f"  Napari:   ({napari_coords[0]}, {napari_coords[1]}, {napari_coords[2]}) pixels"
        )
    ...
    if enable_simulated_data:
        ...
        for z_idx, z_mm in enumerate(z_positions_mm):
            ...
            if z_idx % 2 == 0:
                ...
                app.processEvents()
                print(
                    f"  Processed Z-plane {z_idx + 1}/{num_z_planes} at {z_mm:.2f} mm"
                )
