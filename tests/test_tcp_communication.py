...
if __name__ == "__main__":
    ...
    def test_send_command_structure(self):
        ...
        with self._patch_thread_manager(), self._patch_create_socket_success():
            ...
            self.assertFalse(
                data_q.empty(), "command_data queue should contain an entry"
            )
            ...
            self.assertTrue(
                self.event_manager.is_set("send"), "'send' event should be set"
            )
    ...
    def test_send_workflow(self):
        ...
        with self._patch_thread_manager(), self._patch_create_socket_success():
            ...
            with patch(
                "src.py2flamingo.utils.file_handlers.dict_to_workflow"
            ) as mock_wf_write:
                ...
                self.assertFalse(
                    cmd_q.empty(),
                    "command queue should contain an entry after send_workflow",
                )
                self.assertFalse(
                    data_q.empty(),
                    "command_data queue should contain workflow path or blob",
                )

                # Assert send event set
                self.assertTrue(
                    self.event_manager.is_set("send"),
                    "'send' event should be set after send_workflow",
                )
