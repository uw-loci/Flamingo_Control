...
def safe_move_via_ui(sample_view, axis, position, callback, retry_count=0):
    ...
    try:
        ...
        if "already in progress" in str(e).lower() and retry_count < max_retries:
            ...
            print(
                f"    (waiting for previous motion to finish, retry {retry_count + 1})"
            )
            QTimer.singleShot(
                500,
                lambda: safe_move_via_ui(
                    sample_view, axis, position, callback, retry_count + 1
                ),
            )
        else:
...
def wait_for_motion_complete(position_controller, callback, poll_count=0):
    ...
    if not is_moving:
        ...
        print(
            f"    WARNING: Motion wait timeout ({MOTION_WAIT_TIMEOUT/1000}s), proceeding..."
        )
        QTimer.singleShot(MOTION_SETTLE_DELAY, callback)
    else:
        ...
        QTimer.singleShot(
            ...
            lambda: wait_for_motion_complete(
                position_controller, callback, poll_count + 1
            ),
        )
...
def test_3d_voxel_rotation(app):
    ...
    def step4_move_to_start_position():
        ...
        print(
            f"  Target: X={TEST_X_POSITION}, Y={TEST_Y_START}, Z={TEST_Z_POSITION}, R=0°"
        )
        ...
        def verify_position():
            ...
            if position:
                print(
                    f"  Actual position: X={position.x:.3f}, Y={position.y:.3f}, "
                    f"Z={position.z:.3f}, R={position.r:.1f}°"
                )
            QTimer.singleShot(1000, step5_start_live_view)
    ...
    def step5_start_live_view():
        ...
        if sample_view and hasattr(sample_view, "live_view_toggle_btn"):
            ...
            if (
                ...
                and not sample_view.camera_controller.is_live_view_active()
            ):
        ...
        def move_next_y():
            if current_step[0] < len(y_positions):
                ...
                safe_move_via_ui(
                    ...
                    "y",
                    y_pos,
                    lambda: QTimer.singleShot(1000, move_next_y),
                )
            else:
                        ...
                        if count > 0:
                            ...
                            if len(nonzero_indices[0]) > 0:
                                ...
                                x_range = (
                                    ...
                                    nonzero_indices[2].max(),
                                )
                                print(f"  Channel {ch_id}: {count} voxels")
                                print(
                                    f"    Bounding box: Z={z_range}, Y={y_range}, X={x_range}"
                                )
        else:
        ...
        if app.sample_3d_visualization_window:
            ...
            if hasattr(viz, "voxel_storage") and viz.voxel_storage:
                ...
                for ch_id in range(4):
                    ...
                    if stage_pos:
                        volume = storage.get_display_volume_transformed(
                            ch_id, stage_pos
                        )
                    else:
                        volume = storage.get_display_volume(ch_id)
                    ...
                    if volume is not None:
                        ...
                        if count > 0:
                            ...
                            if len(nonzero_indices[0]) > 0:
                                ...
                                print(
                                    f"  Channel {ch_id}: {count} voxels (was {voxel_counts_before.get(ch_id, 0)})"
                                )
                                print(
                                    f"    Bounding box: Z={z_range}, Y={y_range}, X={x_range}"
                                )
                ...
                for ch_id in set(
                    list(voxel_counts_before.keys()) + list(voxel_counts_after.keys())
                ):
                    ...
                    if before > 0 or after > 0:
                        ...
                        print(
                            f"    Channel {ch_id}: {before} -> {after} ({pct_change:+.1f}%) [{status}]"
                        )
    ...
    def step14_return_to_origin():
        ...
        print(
            f"  Moving to: X={initial_position['x']:.3f}, Y={initial_position['y']:.3f}, "
        ...
        if hasattr(viz_window, "voxel_storage") and viz_window.voxel_storage:
            ...
            for ch_id in range(4):
                ...
                if volume is not None and np.count_nonzero(volume) > 0:
                    ...
                    print(
                        f"      Shape: {volume.shape}, Non-zero: {np.count_nonzero(volume)}"
                    )
            ...
            metadata = {
                ...
                "voxel_size_um": (
                    list(storage.config.display_voxel_size)
